...
public class FirebaseUtil
{
    ...
    {
    	FirebaseStatus status = FirebaseStatus.FAILURE;
        ...
        try
        {
            ...
            if (check == success) status = FirebaseStatus.SUCCESS;
        } catch (Exception e) { e.printStackTrace (); }
    	
    	return status;
    }
    
    public static FirebaseStatus send (String token, String title, String body, String link)
    {
    	FirebaseStatus status = FirebaseStatus.FAILURE;
    ...
    public static FirebaseStatus send (List <String> token, String title, String body)
    {
    	FirebaseStatus status = FirebaseStatus.FAILURE;
        ...
        try
        {
            ...
            if (check == success) status = FirebaseStatus.SUCCESS;
            if (check > success && 0 < success) status = FirebaseStatus.PARTIAL;
        } catch (Exception e) { e.printStackTrace (); }

        return status;
    }    
    
    public static FirebaseStatus send (List <String> token, String title, String body, String link)
    {
    	FirebaseStatus status = FirebaseStatus.FAILURE;
